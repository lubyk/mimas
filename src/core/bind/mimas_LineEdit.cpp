 ...
 * bind/QWidget.h:55
 */
static int LineEdit_setCssClass(lua_State *L) {
  try {
    LineEdit *self = *((LineEdit **)dub_checksdata(L, 1, "mimas.LineEdit"));
 ...
 * bind/QWidget.h:56
 */
static int LineEdit_setSizeHit(lua_State *L) {
  try {
    LineEdit *self = *((LineEdit **)dub_checksdata(L, 1, "mimas.LineEdit"));
    int w = dub_checkint(L, 2);
    int h = dub_checkint(L, 3);
...
static int LineEdit_text(lua_State *L) {
  try {
    LineEdit *self = *((LineEdit **)dub_checksdata(L, 1, "mimas.LineEdit"));
    QByteArray s_self->text()_(self->text().toUtf8());
    lua_pushlstring(L, s_self->text()_.constData(), s_self->text()_.size());
    return 1;
  } catch (std::exception &e) {
...
static const struct luaL_Reg LineEdit_member_methods[] = {
  ...
  { "setCssClass"  , LineEdit_setCssClass },
  { "setSizeHit"   , LineEdit_setSizeHit  },
